...
function(app, Backbone) {
  ...
  Views.WallView = Backbone.View.extend({
    ...
    initialize: function() {
      ...
      //   console.log("WallView.initialize(): model changed", this);
      //   this.render();
      // });
    ...
    render: function() {
      console.log("WallView.render():");
    }
  });

  // // FormView (survey and results)
  // // ----------
...
function(app, Backbone, Views) {
  ...
  Wall.SurveyModel = Backbone.Model.extend({
    ...
    url: "services/GetSurvey.php", // + survey id

    parse: function(response) {

      // preprocess filters into a collection before returning response
      response.survey.filters = new Wall.FilterList(response.survey.filters); 
  ...
  // -------------------------------------------------------------
  // Wall consists of title, labels for axes, and the items which make up the wall
  Wall.WallModel = Backbone.Model.extend({

    url: "services/GetWall.php", // + wall id

    parse: function(response) {

      // preprocess items into a collection before returning response
      response.wall.items = new Wall.ItemList(response.wall.items);
  ...
  // Item list
  Wall.ItemList = Backbone.Collection.extend({ model: Wall.ItemModel });
...
function(app, Wall) {
  ...
  var Router = Backbone.Router.extend({
    routes: {
      "survey/id:id"  : "survey", // #/survey/id64
      "wall"    : "wall",
      "results" : "results"
    },
    ...
    survey: function(id) {
      ...
      var surveyModel = new Wall.SurveyModel();
      surveyModel.on('change', function() {
        console.log("SurveyModel changed", this.get('filters').at(0).get('type'));
      });
      surveyModel.fetch();
    },

    wall: function() {
      console.log("Wall route fired");
      var wallModel = new Wall.WallModel();

      app.useLayout("main").setViews({
        // Create a new WallView in the wall class of 
        // the main layout.
    ...
    results: function() {
      console.log("Results route fired");
    }
  });
...
function(app, Router) {
  ...
  // http://stackoverflow.com/questions/10155660/backbone-js-and-leading-hash
  // Removed pushState: true to stop backbone removing hash and subsequent page
  // loads not working correctly. 
